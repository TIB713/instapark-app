...
export function useDriverTasks(
  ...
  const [takingHandoverPhoto, setTakingHandoverPhoto] = useState(null);
  ...
  const fetchRetrievals = useCallback(async () => {
    try {
      ...
      if (!hasSeededSeenRef.current) {
        ...
        fetchedCars.forEach((car) => maybeQueueNewRequestRef.current(car));
      }
    } catch { }
...
const takeHandoverPhoto = async (car) => {
  setTakingHandoverPhoto(car.id);

  const perm = await ImagePicker.requestCameraPermissionsAsync();
  if (!perm.granted) {
    setTakingHandoverPhoto(null);
  ...
  setHandoverPhotos((prev) => ({ ...prev, [car.id]: finalUri }));
};

const removeHandoverPhoto = (car) => {
  setHandoverPhotos((prev) => {
    const next = { ...prev };
    delete next[car.id];
    return next;
  });
...
const doMarkHandover = async (car) => {
  setHandoverUploading(true);
  const finalUri = handoverPhotos[car.id] || null;
  await doHandleHandover(car, finalUri);
  setHandoverPhotos((prev) => {
    const next = { ...prev };
    delete next[car.id];
    return next;
...
const doHandleHandover = async (car, finalUri) => {
  ...
  if (!net.isConnected) {
    if (!finalUri) {
      try {
        await enqueueHandover(car.id, null);
        await refreshPending();
        confirmDialog.info("Saved offline", "Handover saved. Will sync when connected.");
