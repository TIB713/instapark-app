...
if target_server in content:
    ...
    print("Warning: Backend patch target not found")
...
# Add state variables
target_hook_state = """const [showCarModal, setShowCarModal] = useState(false);"""
replacement_hook_state = """const [showCarModal, setShowCarModal] = useState(false);
...
if target_hook_state in content and "selfPickupOtpInput" not in content:
    content = content.replace(target_hook_state, replacement_hook_state)
...
# Add exports
target_hook_exports = """    showCarModal, setShowCarModal,"""
replacement_hook_exports = """    showCarModal, setShowCarModal,
    selfPickupOtpInput, setSelfPickupOtpInput,
    showSelfPickupOtpField, setShowSelfPickupOtpField,"""
if target_hook_exports in content and "selfPickupOtpInput" not in content.split("return {")[1]:
    content = content.replace(target_hook_exports, replacement_hook_exports)
...
print("Done apply_self_pickup.py")
