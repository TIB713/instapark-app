...
const EntryGateSection = memo(({ eventGates, selectedGate, setSelectedGate }) => {
  ...
  return (
    <>
      <Lbl>ENTRY GATE</Lbl>
      <ScrollView ref={scrollViewRef} horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: rp(8), paddingBottom: rp(4) }} style={{ marginBottom: rp(12) }}>
        {eventGates.map((g) => (
          <TouchableOpacity
            key={g}
...
const PhotoGridSection = memo(({ photos, errors, takePhoto, onRemovePhoto }) => {
  return (
    <>
      ...
      <View ref={el => { if (fieldRefs.current) fieldRefs.current.photos = el; }}  style={{ flexDirection: "row", flexWrap: "wrap", gap: rp(10), marginBottom: errors.photos ? 0 : rp(16), borderWidth: errors.photos ? rp(1) : 0, borderColor: theme.colors.danger, borderRadius: rp(16), padding: errors.photos ? rp(8) : 0 }}>
...
export default function AddCar() {
  ...
  const submit = async () => {
    ...
    if (Object.keys(errs).length > 0) {
      setSubmitting(false);
      scrollToFirstError(['plate', 'color', 'make', 'guestName', 'driver', 'guestPhone', 'altGuestPhone', 'photos'], errs, fieldRefs, scrollViewRef);
      return;
    }
            ...
            {loadingDrivers ? (
              ...
              <View ref={el => { if (fieldRefs.current) fieldRefs.current.driver = el; }}  style={{ backgroundColor: theme.colors.surface, borderRadius: rp(16), borderWidth: rp(1), borderColor: errors.driver ? theme.colors.danger : theme.colors.border, overflow: "hidden" }}>
                {drivers.map((d, idx) => {
                  ...
                  return (
                    <TouchableOpacity ref={el => { if (fieldRefs.current) fieldRefs.current.driver = el; }}  key={d.id} onPress={() => { handlePickDriver(d); if (errors.driver) setErrors(prev => ({ ...prev, driver: undefined })); }} style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingVertical: rp(14), paddingHorizontal: rp(16), borderTopWidth: idx === 0 ? 0 : rp(1), borderTopColor: theme.colors.border, backgroundColor: selected ? theme.colors.successLight : theme.colors.surface }}>
                      <View style={{ flex: 1 }}>
