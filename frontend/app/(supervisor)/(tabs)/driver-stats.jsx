...
export default function SupervisorDriverStats() {
          ...
          <View style={{ paddingHorizontal: rp(theme.spacing.lg), paddingTop: rp(theme.spacing.sm), paddingBottom: rp(theme.spacing.sm) }}>
            <View style={{ flexDirection: "row", backgroundColor: theme.colors.surfaceAlt, borderRadius: rp(theme.radius.md), padding: rp(4) }}>
              {["overview", "events"].map((k) => (
                ...
                >
                  <Text style={{ fontWeight: theme.fontWeight.bold, fontSize: rs(theme.fontSize.caption), color: tab === k ? theme.colors.textPrimary : theme.colors.textSecondary, textTransform: "uppercase", letterSpacing: rs(0.5) }}>
                    {k}
                  </Text>
                </TouchableOpacity>
              ))}
