...
function statusMeta(duty_status) {
  if (duty_status === "available") return { color: "#059669", bg: "#D1FAE5", label: "Available" };
  if (duty_status === "busy") return { color: "#D97706", bg: "#FEF3C7", label: "Busy" };
  return { color: "#6B7280", bg: "#F3F4F6", label: "Offline" };
}
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#ECFDF5" }}>
      <SafeAreaView edges={["top"]} style={{ backgroundColor: "#059669" }}>
        <View style={{ backgroundColor: "#059669", borderBottomLeftRadius: 44, borderBottomRightRadius: 44, paddingHorizontal: rp(20), paddingTop: rp(8), paddingBottom: rp(24) }}>
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            ...
            <Text style={{ color: "#fff", fontSize: rs(20), fontWeight: "900", marginLeft: rp(12), flex: 1 }}>
              Add Car & Assign Driver
            </Text>
            <TouchableOpacity
              onPress={() => router.push({ pathname: "/(driver)/scanner", params: { returnTo: "/(supervisor)/add-car" } })}
        ...
        <ScrollView style={{ flex: 1, paddingHorizontal: rp(20), paddingTop: rp(18) }} keyboardShouldPersistTaps="handled" contentContainerStyle={{ paddingBottom: rp(100) }}>
          {isPreRegistered && (
            <View style={{ backgroundColor: "#ECFDF5", borderWidth: rp(1), borderColor: "#6EE7B7", borderRadius: rp(16), padding: rp(12), marginBottom: rp(16), flexDirection: "row", alignItems: "center", gap: rp(10) }}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: rs(12), fontWeight: "900", color: "#059669" }}>PRE-REGISTERED GUEST</Text>
                {guestName ? (
                  <Text style={{ fontSize: rs(11), color: "#065F46", marginTop: rp(1) }}>{guestName} — details pre-filled, please verify</Text>
                ) : null}
              </View>
            </View>
          )}
          {isPreRegistered && guestNotes ? (
            <View style={{ backgroundColor: "#FEF3C7", borderWidth: rp(1), borderColor: "#FDE68A", borderRadius: rp(16), padding: rp(12), marginBottom: rp(16), flexDirection: "row", alignItems: "flex-start", gap: rp(10) }}>
              <Ionicons name="information-circle" size={20} color="#D97706" style={{ marginTop: rp(1) }} />
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: rs(12), fontWeight: "900", color: "#92400E" }}>GUEST INSTRUCTIONS</Text>
                <Text style={{ fontSize: rs(13), color: "#78350F", marginTop: rp(4), lineHeight: 18 }}>{guestNotes}</Text>
              </View>
            </View>
          ) : null}
            ...
            <TextInput
              ...
              onChangeText={(v) => {
                ...
                if (cleaned === "") {
                  ...
                  if (lookupApplied) {
                    setMake(""); setColor(""); setGuestName(""); setGuestPhone(""); setAltGuestPhone(""); setCarType("normal"); setLookupApplied(false);
                  }
                }
              }}
            ...
            <View style={{ backgroundColor: "#ECFDF5", borderWidth: rp(1), borderColor: "#6EE7B7", borderRadius: rp(16), padding: rp(12), marginBottom: rp(16) }}>
              <View style={{ flexDirection: "row", alignItems: "center", gap: rp(10) }}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={{ fontSize: rs(11), color: "#065F46", marginTop: rp(1) }}>{pendingLookup.guest_name ? `${pendingLookup.guest_name} — ` : ""}Use these saved details?</Text>
                </View>
              </View>
          ...
          {lookupApplied && !pendingLookup && (
            <View style={{ marginBottom: rp(16) }}>
              <Text style={{ fontSize: rs(11), color: "#059669", marginBottom: rp(8) }}>✓ Details filled from previous visit</Text>
              <TouchableOpacity onPress={clearGuestOnly} style={{ borderWidth: rp(1), borderColor: "#FCA5A5", backgroundColor: "#FEF2F2", borderRadius: rp(10), paddingVertical: rp(8), paddingHorizontal: rp(14), alignSelf: "flex-start", flexDirection: "row", alignItems: "center", gap: rp(6) }}>
                <Ionicons name="person-remove-outline" size={14} color="#DC2626" />
                <Text style={{ fontSize: rs(12), fontWeight: "800", color: "#DC2626" }}>Not this guest? Clear name &amp; phone</Text>
              </TouchableOpacity>
            </View>
          )}
          ...
          <View style={{ flexDirection: "row", gap: rp(8), marginBottom: rp(16) }}>
            {["normal", "premium"].map((ct) => (
              <TouchableOpacity key={ct} onPress={() => setCarType(ct)} style={{ paddingHorizontal: rp(14), paddingVertical: rp(8), borderRadius: rp(99), backgroundColor: carType === ct ? "#059669" : "#fff", borderWidth: rp(1), borderColor: "#059669" }}>
          ...
          <View style={[inputRow, { alignItems: "flex-start", paddingTop: rp(12) }]}>
            ...
            <TextInput value={notes} onChangeText={setNotes} multiline placeholder="Special notes..." placeholderTextColor="#9CA3AF" style={[textInput, { minHeight: 60, textAlignVertical: "top" }]} />
          </View>
            ...
            <>
              ...
              <View style={{ flexDirection: "row", flexWrap: "wrap", gap: rp(8), marginBottom: rp(16) }}>
                {DAMAGE_OPTIONS.map((opt) => {
                  ...
                  return (
                    <TouchableOpacity key={opt} onPress={() => setDamageTypes(prev => selected ? prev.filter(t => t !== opt) : [...prev, opt])} style={{ paddingHorizontal: rp(14), paddingVertical: rp(8), borderRadius: rp(99), backgroundColor: selected ? "#059669" : "#fff", borderWidth: rp(1), borderColor: "#059669" }}>
                      <Text style={{ fontSize: rs(12), fontWeight: "800", color: selected ? "#fff" : "#059669", letterSpacing: rs(0.5) }}>{opt}</Text>
                    </TouchableOpacity>
                  );
                })}
                <TouchableOpacity onPress={() => setShowOtherDamage(!showOtherDamage)} style={{ paddingHorizontal: rp(14), paddingVertical: rp(8), borderRadius: rp(99), backgroundColor: showOtherDamage ? "#059669" : "#fff", borderWidth: rp(1), borderColor: "#059669" }}>
              ...
              {showOtherDamage && (
                <>
                  ...
                  <View style={[inputRow, { alignItems: "flex-start", paddingTop: rp(12) }]}>
                    ...
                    <TextInput value={damageNotes} onChangeText={setDamageNotes} multiline placeholder="Describe scratches, dents, etc..." placeholderTextColor="#9CA3AF" style={[textInput, { minHeight: 60, textAlignVertical: "top" }]} />
                  </View>
                </>
              )}
            </>
          ...
          <View style={[inputRow, errors.guestPhone && { borderColor: "#EF4444", marginBottom: 0 }]}>
            ...
            <TextInput value={guestPhone} onChangeText={(text) => { setGuestPhone(text); if (errors.guestPhone) setErrors(prev => ({ ...prev, guestPhone: undefined })); }} placeholder="10-digit mobile number" placeholderTextColor="#9CA3AF" keyboardType="phone-pad" maxLength={10} style={textInput} />
          </View>
          ...
          <View style={[inputRow, errors.altGuestPhone && { borderColor: "#EF4444", marginBottom: 0 }]}>
            <Ionicons name="phone-portrait-outline" size={20} color="#059669" />
            <TextInput value={altGuestPhone} onChangeText={(text) => { setAltGuestPhone(text); if (errors.altGuestPhone) setErrors(prev => ({ ...prev, altGuestPhone: undefined })); }} placeholder="10-digit mobile number" placeholderTextColor="#9CA3AF" keyboardType="phone-pad" maxLength={10} style={textInput} />
          </View>
          ...
          {eventGates.length > 0 && (
            <>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: rp(8), paddingBottom: rp(4) }} style={{ marginBottom: rp(12) }}>
                {eventGates.map((g) => (
                  <TouchableOpacity key={g} onPress={() => setSelectedGate(g)} style={{ paddingHorizontal: rp(14), paddingVertical: rp(8), borderRadius: rp(99), backgroundColor: selectedGate === g ? "#059669" : "#fff", borderWidth: rp(1), borderColor: "#059669" }}>
              ...
              <View key={label} style={{ width: rp(80), height: rp(80) }}>
                {photos[label] ? (
                  <>
                    ...
                    <TouchableOpacity onPress={() => setPhotos({ ...photos, [label]: null })} style={{ position: "absolute", top: rp(-6), right: rp(-6), backgroundColor: "rgba(255, 255, 255, 0.8)", borderRadius: rp(99), padding: rp(2) }}>
                      <Ionicons name="close-circle" size={24} color="#EF4444" />
                    </TouchableOpacity>
                  </>
                ) : (
                  <TouchableOpacity onPress={() => takePhoto(label)} style={{ width: rp(80), height: rp(80), borderRadius: rp(16), borderWidth: rp(2), borderColor: "#059669", borderStyle: "dashed", alignItems: "center", justifyContent: "center", backgroundColor: "#fff" }}>
          ...
          {loadingDrivers ? (
            <ActivityIndicator style={{ marginTop: rp(10), marginBottom: rp(16) }} color="#059669" />
          ) : drivers.length === 0 ? (
            <Text style={{ color: "#9CA3AF", fontSize: rs(13), marginBottom: rp(16) }}>No drivers rostered on this event yet.</Text>
          ) : (
            <View style={{ backgroundColor: "#fff", borderRadius: rp(16), borderWidth: rp(1), borderColor: errors.driver ? "#EF4444" : "#E5E7EB", overflow: "hidden", marginBottom: errors.driver ? 0 : rp(16) }}>
              {drivers.map((d, idx) => {
                const meta = statusMeta(d.duty_status);
                const selected = selectedDriverId === d.id;
                return (
                  <TouchableOpacity key={d.id} onPress={() => { handlePickDriver(d); if (errors.driver) setErrors(prev => ({ ...prev, driver: undefined })); }} style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingVertical: rp(14), paddingHorizontal: rp(16), borderTopWidth: idx === 0 ? 0 : rp(1), borderTopColor: "#F3F4F6", backgroundColor: selected ? "#ECFDF5" : "#fff" }}>
                    <View style={{ flex: 1 }}>
                      <Text style={{ fontWeight: "800", color: "#111827", fontSize: rs(14) }}>{d.name}</Text>
                      {d.duty_status === "busy" && d.current_car_plate && (
                        <Text style={{ color: "#9CA3AF", fontSize: rs(11), marginTop: rp(2) }}>Busy with {d.current_car_plate}</Text>
                      )}
                    </View>
                    <View style={{ backgroundColor: meta.bg, paddingHorizontal: rp(10), paddingVertical: rp(4), borderRadius: rp(99), marginRight: rp(10) }}>
                      <Text style={{ color: meta.color, fontWeight: "800", fontSize: rs(10) }}>{meta.label}</Text>
                    </View>
                    {selected && <Ionicons name="checkmark-circle" size={22} color="#059669" />}
                  </TouchableOpacity>
                );
              })}
            </View>
