        ...
        <ScrollView className="flex-1 px-4 pt-4">
          <View className="flex-row gap-3 mb-4">
            <View className="flex-1 bg-white rounded-2xl p-4">
              ...
              <Text className="text-3xl font-black text-[#0F2044] mt-1">{stats.cars_checked_in}</Text>
            </View>
            <View className="flex-1 bg-white rounded-2xl p-4">
              <Text className="text-xs font-bold text-gray-500 tracking-widest">RETRIEVED</Text>
              <Text className="text-3xl font-black text-[#0F2044] mt-1">{stats.cars_retrieved}</Text>
            </View>
          </View>
          ...
          <Text className="text-lg font-black text-[#0F2044] mt-6 mb-2">Edit Driver</Text>
          <View className="bg-white rounded-2xl p-4">
            ...
            <TouchableOpacity onPress={saveDriver} className="bg-[#0F2044] rounded-2xl py-3 items-center">
        ...
        <ScrollView className="flex-1 px-4 pt-4">
          <ScrollView horizontal contentContainerStyle={{ gap: 8 }} showsHorizontalScrollIndicator={false}>
            {["all", "active", "closed"].map((f) => (
              <TouchableOpacity key={f} onPress={() => setEvtFilter(f)} className={`px-4 py-2 rounded-full ${evtFilter === f ? "bg-[#0F2044]" : "bg-white border border-gray-200"}`}>
                <Text className={`text-xs font-bold ${evtFilter === f ? "text-white" : "text-gray-600"}`}>{f.toUpperCase()}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
          {loadingEvents && <ActivityIndicator color="#0F2044" style={{ marginTop: 20 }} />}
