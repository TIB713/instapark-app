...
import { Screen, Btn, Field, FieldLabel, fieldTextInputStyle } from "../../../components/valet/ui";
...
export default function DriverNew() {
  ...
  const saveDriver = async () => {
    ...
    try {
      ...
      router.replace("/(admin)/(tabs)/manage-employees?tab=drivers");
    } catch (e) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.colors.surfaceAlt }} testID="add-driver-screen">
      <Hero eyebrow="People" title="Add driver" onBack={() => router.replace("/(admin)/(tabs)/manage-employees?tab=drivers")} />
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
        <ScrollView ref={scrollViewRef} keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: rp(theme.spacing.xl), paddingBottom: rp(theme.spacing.xxxl) + (insets?.bottom || 0) + tabBarHeight }}>
          ...
          <TouchableOpacity onPress={pickDriverPhoto} style={{ alignItems: "center", marginBottom: rp(theme.spacing.lg) }}>
            <FieldLabel style={{ textAlign: "center" }}>Driver Photo (optional)</FieldLabel>
            {drvPhotoUri ? (
          ...
          <Field error={driverErrors.name}>
            <TextInput ref={el => { if (fieldRefs.current) fieldRefs.current.name = el; }} value={drvName} onChangeText={(t) => { setDrvName(t); if (driverErrors.name) setDriverErrors(prev => ({ ...prev, name: undefined })); }} placeholder="Full Name" style={fieldTextInputStyle} />
          </Field>
          ...
          <Field error={driverErrors.phone}>
            <TextInput ref={el => { if (fieldRefs.current) fieldRefs.current.phone = el; }} value={drvPhone} onChangeText={(t) => { setDrvPhone(t); if (driverErrors.phone) setDriverErrors(prev => ({ ...prev, phone: undefined })); }} maxLength={10} placeholder="10-digit mobile" keyboardType="phone-pad" style={fieldTextInputStyle} />
          </Field>
          ...
          <View style={{ flexDirection: 'row', gap: rp(10), marginBottom: rp(theme.spacing.lg) }}>
            ...
            >
              <Text style={{ fontFamily: theme.fontFamily.semibold, fontWeight: '600', color: drvGender === 'male' ? theme.colors.info : theme.colors.textSecondary, fontSize: rp(14) }}>Male</Text>
            </TouchableOpacity>
            <TouchableOpacity
            ...
            >
              <Text style={{ fontFamily: theme.fontFamily.semibold, fontWeight: '600', color: drvGender === 'female' ? theme.colors.info : theme.colors.textSecondary, fontSize: rp(14) }}>Female</Text>
            </TouchableOpacity>
          </View>
          ...
          <Field error={driverErrors.email}>
            <TextInput ref={el => { if (fieldRefs.current) fieldRefs.current.email = el; }} value={drvEmail} onChangeText={(t) => { setDrvEmail(t); if (driverErrors.email) setDriverErrors(prev => ({ ...prev, email: undefined })); }} placeholder="driver@example.com" autoCapitalize="none" keyboardType="email-address" style={fieldTextInputStyle} />
          </Field>
          {driverErrors.email && <Text style={modalErrorText}>* {driverErrors.email}</Text>}
          <FieldLabel>PAN CARD NUMBER (OPTIONAL)</FieldLabel>
          <Field icon="card-outline" error={driverErrors.pan}>
            <TextInput ref={el => { if (fieldRefs.current) fieldRefs.current.pan = el; }} value={drvPan} onChangeText={(v) => { setDrvPan(v.toUpperCase()); if (driverErrors.pan) setDriverErrors(prev => ({ ...prev, pan: undefined })); }} placeholder="ABCDE1234F" autoCapitalize="characters" maxLength={10} style={fieldTextInputStyle} />
          </Field>
          {driverErrors.pan && <Text style={modalErrorText}>* {driverErrors.pan}</Text>}
          <FieldLabel>BANK ACCOUNT NUMBER (OPTIONAL)</FieldLabel>
          <Field icon="business-outline" error={driverErrors.bankAccount}>
            <TextInput ref={el => { if (fieldRefs.current) fieldRefs.current.bankAccount = el; }} value={drvBankAccount} onChangeText={(t) => { setDrvBankAccount(t); if (driverErrors.bankAccount) setDriverErrors(prev => ({ ...prev, bankAccount: undefined })); }} placeholder="Account number" keyboardType="numeric" maxLength={18} style={fieldTextInputStyle} />
          </Field>
          ...
          <Field icon="business-outline" error={driverErrors.bankIfsc}>
          ...
          {drvIfscChecking && <Text style={{ fontFamily: theme.fontFamily.regular, color: theme.colors.textMuted, fontSize: rs(12), marginTop: rp(-12), marginBottom: rp(theme.spacing.lg) }}>Checking IFSC...</Text>}
          ...
          <FieldLabel>DRIVING LICENCE NUMBER <Text style={{ fontFamily: theme.fontFamily.regular, color: theme.colors.danger }}>*</Text></FieldLabel>
          <Field error={driverErrors.licenseNumber}>
          ...
          <TouchableOpacity ref={el => { if (fieldRefs.current) fieldRefs.current.licensePhoto = el; }} onPress={() => { pickLicensePhoto(); if (driverErrors.licensePhoto) setDriverErrors(prev => ({ ...prev, licensePhoto: undefined })); }} style={{ alignItems: "center", marginBottom: rp(theme.spacing.lg) }}>
            <FieldLabel style={{ textAlign: "center" }}>Licence Photo <Text style={{ fontFamily: theme.fontFamily.regular, color: theme.colors.danger }}>*</Text></FieldLabel>
            {drvLicensePhotoUri ? (
              <View style={{ position: "relative" }}>
          ...
          {driverErrors.licensePhoto && <Text style={[modalErrorText, { fontFamily: theme.fontFamily.regular, textAlign: 'center' }]}>* {driverErrors.licensePhoto}</Text>}

          <FieldLabel>AADHAR NUMBER <Text style={{ fontFamily: theme.fontFamily.regular, color: theme.colors.danger }}>*</Text></FieldLabel>
          <Field error={driverErrors.aadharNumber}>
            <TextInput ref={el => { if (fieldRefs.current) fieldRefs.current.aadharNumber = el; }} value={drvAadharNumber} onChangeText={(v) => { const digits = v.replace(/\D/g, "").slice(0, 12); setDrvAadharNumber(digits); if (driverErrors.aadharNumber) setDriverErrors(prev => ({ ...prev, aadharNumber: undefined })); }} placeholder="Aadhar number" keyboardType="numeric" maxLength={12} style={fieldTextInputStyle} />
          </Field>
          ...
          {driverErrors.aadharPhoto && <Text style={[modalErrorText, { fontFamily: theme.fontFamily.regular, textAlign: 'center' }]}>* {driverErrors.aadharPhoto}</Text>}


          <Btn variant="primary" onPress={saveDriver} disabled={savingDrv} style={{ height: rp(56), marginTop: rp(theme.spacing.xl) }}>
            {savingDrv ? <ActivityIndicator color={theme.colors.surface} /> : (
              <View style={{ flexDirection: "row", alignItems: "center" }}>
                <Ionicons name="checkmark" size={rs(20)} color={theme.colors.surface} style={{ marginRight: rp(8) }} />
                <Text style={{ fontFamily: theme.fontFamily.bold, color: theme.colors.surface, fontWeight: theme.fontWeight.bold, fontSize: rs(theme.fontSize.bodyLarge) }}>Save Driver</Text>
              </View>
            )}
          </Btn>
