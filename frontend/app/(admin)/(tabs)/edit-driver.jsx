...
export default function DriverEdit() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.colors.surfaceAlt }} testID="edit-driver-screen">
      <Hero eyebrow="People" title="Edit driver" onBack={() => router.replace("/(admin)/(tabs)/manage-employees?tab=drivers")} />
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
        <ScrollView ref={scrollViewRef} keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: rp(theme.spacing.xl), paddingBottom: rp(theme.spacing.xxxl) + (insets?.bottom || 0) + tabBarHeight }}>
          ...
          <Field error={driverErrors.aadharNumber}>
            <TextInput ref={el => { if (fieldRefs.current) fieldRefs.current.aadharNumber = el; }} value={drvAadharNumber} onChangeText={(v) => { const digits = v.replace(/\D/g, "").slice(0, 12); setDrvAadharNumber(digits); if (driverErrors.aadharNumber) setDriverErrors(prev => ({ ...prev, aadharNumber: undefined })); }} placeholder="12-digit Aadhar" keyboardType="numeric" maxLength={12} style={fieldTextInputStyle} />
          </Field>
