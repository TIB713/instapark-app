...
export default function SupervisorNew() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.colors.surfaceAlt }} testID="add-supervisor-screen">
      <Hero eyebrow="People" title="Add supervisor" onBack={() => router.replace("/(admin)/(tabs)/manage-employees?tab=supervisors")} />
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
        <ScrollView ref={scrollViewRef} keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: rp(theme.spacing.xl), paddingBottom: rp(theme.spacing.xxxl) + (insets?.bottom || 0) + tabBarHeight }}>
          ...
          <Btn variant="primary" onPress={saveSupervisor} disabled={savingSup} style={{ height: rp(56), marginTop: rp(theme.spacing.xl) }}>
            {savingSup ? <ActivityIndicator color={theme.colors.surface} /> : (
              <View style={{ flexDirection: "row", alignItems: "center" }}>
                ...
                <Text style={{ fontFamily: theme.fontFamily.bold, color: theme.colors.surface, fontWeight: theme.fontWeight.bold, fontSize: rs(theme.fontSize.bodyLarge) }}>Save Supervisor</Text>
              </View>
            )}
          </Btn>
