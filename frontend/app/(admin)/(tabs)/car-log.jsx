...
const STATUS_CONFIG = {
  REGISTERED: { color: theme.colors.info, icon: "document-text-outline", label: "Registered" },
  PRE_REGISTERED: { color: theme.colors.primary, icon: "time-outline", label: "Pre-Registered" },
  CHECKED_IN: { color: theme.colors.info, icon: "log-in-outline", label: "Checked In" },
  PARKED: { color: theme.colors.success, icon: "car-outline", label: "Parked" },
  RETRIEVAL_REQUESTED: { color: theme.colors.warning, icon: "notifications-outline", label: "Retrieval Requested" },
  ACCEPTED: { color: theme.colors.warning, icon: "checkmark-circle-outline", label: "Retrieval Accepted" },
  BEING_FETCHED: { color: theme.colors.warning, icon: "walk-outline", label: "Being Fetched" },
  ARRIVED_AT_GATE: { color: theme.colors.success, icon: "location-outline", label: "Arrived At Gate" },
  ...
  return (
    <View style={{ flexDirection: "row" }}>
      ...
      <View style={{ width: rp(40), alignItems: "center" }}>
        <View style={{
          width: rp(36), height: rp(36), borderRadius: rp(18),
        ...
        {!isLast && (
          <View style={{
            width: rp(2), flex: 1, minHeight: 24,
            backgroundColor: theme.colors.border, marginTop: rp(2)
          }} />
        )}
      </View>
      ...
      <View style={{
        ...
        <View style={{
          ...
          <Text style={{
            fontFamily: theme.fontFamily.bold, fontWeight: "900", fontSize: rs(15),
            color: theme.colors.textPrimary
          }}>{label}</Text>
          <Text style={{
            fontFamily: theme.fontFamily.bold, fontSize: rs(11), color: theme.colors.textMuted,
            fontWeight: "700"
          }}>{fmt(time)}</Text>
        </View>
        ...
        {driver && (
          <View style={{
            ...
            <Text style={{
              fontFamily: theme.fontFamily.bold, color: theme.colors.textSecondary, fontSize: rs(12),
        ...
        {note ? (
          <View style={{
            ...
            <Text style={{
              fontFamily: theme.fontFamily.regular, color: theme.colors.warning, fontSize: rs(12),
              fontStyle: "italic"
            }}>"{note}"</Text>
          </View>
        ) : null}
        ...
        {durationCaption ? (
          <Text style={{ fontFamily: theme.fontFamily.regular, color: theme.colors.textSecondary, fontSize: rs(12), marginTop: rp(4), fontStyle: "italic" }}>
            {durationCaption}
          </Text>
        ) : null}
    ...
    <View style={{ flexDirection: "row" }}>
      <View style={{ width: rp(40), alignItems: "center" }}>
        <View style={{
          ...
          <Ionicons name="warning" size={18} color={theme.colors.surface} />
        </View>
        {!isLast && (
          <View style={{
            width: rp(2), flex: 1, minHeight: 24,
      ...
      <View style={{
        ...
        <View style={{
          ...
          <Text style={{ fontFamily: theme.fontFamily.regular, color: theme.colors.danger, fontSize: rs(12) }}>
        ...
        {incident.photo_url && (
          <TouchableOpacity
            ...
            <Image source={{ uri: incident.photo_url }}
              style={{
                width: rp(80), height: rp(80), borderRadius: rp(12),
...
export default function CarLog() {
  ...
  if (!log) return (
    <View style={{
      ...
      <Text style={{ fontFamily: theme.fontFamily.regular, color: theme.colors.textSecondary }}>Log not available</Text>
    </View>
  );
  ...
  if (car.retrieval_requested_at) {
    ...
    steps.push({
      type: "status", status: "RETRIEVAL_REQUESTED",
  ...
  if (car.retrieval_driver_id) {
    ...
    steps.push({
      type: "status", status: "BEING_FETCHED",
  ...
  if (car.gate_arrival_time) {
    steps.push({
      type: "status", status: "ARRIVED_AT_GATE",
  ...
  if (car.awaiting_repark_at) {
    steps.push({
      type: "status", status: "AWAITING_REPARK",
  ...
  if (car.status === "DELIVERED") {
    if (car.delivery_type === "self_pickup") {
      steps.push({
        type: "status", status: "SELF_PICKUP",
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.colors.surfaceAlt }}>
          ...
          <View style={{
            ...
            <View style={{ flexDirection: "row", alignItems: "center" }}>
              ...
              <View style={{ marginLeft: rp(14), flex: 1 }}>
                <Text style={{
                  fontFamily: theme.fontFamily.bold, fontSize: rs(22), fontWeight: "900",
                  color: theme.colors.surface
                }}>{car.plate || "No Plate"}</Text>
                {car.has_plate_issue && car.plate && (
                  <Text style={{ fontFamily: theme.fontFamily.bold, color: theme.colors.warning, fontSize: rs(11), fontWeight: "700" }}>TC NUMBER</Text>
                )}
                <Text style={{ fontFamily: theme.fontFamily.regular, color: "rgba(255,255,255,0.7)", fontSize: rs(13) }}>
                  {car.color} {car.make}
                </Text>
              </View>
              <View style={{
            ...
            {car.guest_name && (
              <View style={{
                marginTop: rp(14), paddingTop: rp(14),
                borderTopWidth: rp(1),
            ...
            <View style={{ flexDirection: "row", gap: rp(8), marginTop: rp(16) }}>
              <View style={{ flex: 1, backgroundColor: "rgba(255,255,255,0.12)", borderRadius: rp(16), paddingVertical: rp(12), alignItems: "center" }}>
                <Text style={{ fontFamily: theme.fontFamily.bold, fontSize: rs(20), fontWeight: "900", color: theme.colors.surface }}>{total_minutes ? `${total_minutes}m` : "Active"}</Text>
                <Text style={{ fontFamily: theme.fontFamily.bold, fontSize: rs(9), color: "rgba(255,255,255,0.7)", fontWeight: "800", marginTop: rp(2), letterSpacing: 1 }}>TOTAL TIME</Text>
              </View>
              <View style={{ flex: 1, backgroundColor: "rgba(255,255,255,0.12)", borderRadius: rp(16), paddingVertical: rp(12), alignItems: "center" }}>
                <Text style={{ fontFamily: theme.fontFamily.bold, fontSize: rs(20), fontWeight: "900", color: theme.colors.surface }}>{rating ? `${rating} ⭐` : "—"}</Text>
                <Text style={{ fontFamily: theme.fontFamily.bold, fontSize: rs(9), color: "rgba(255,255,255,0.7)", fontWeight: "800", marginTop: rp(2), letterSpacing: 1 }}>RATING</Text>
              </View>
              <View style={{ flex: 1, backgroundColor: "rgba(255,255,255,0.12)", borderRadius: rp(16), paddingVertical: rp(12), alignItems: "center" }}>
                <Text style={{ fontFamily: theme.fontFamily.bold, fontSize: rs(20), fontWeight: "900", color: theme.colors.surface }}>{incidents.length}</Text>
                <Text style={{ fontFamily: theme.fontFamily.bold, fontSize: rs(9), color: "rgba(255,255,255,0.7)", fontWeight: "800", marginTop: rp(2), letterSpacing: 1 }}>INCIDENTS</Text>
              </View>
            </View>
          </View>
        </SafeAreaView>
      ...
      <ScrollView contentContainerStyle={{
        ...
        <View style={{
          backgroundColor: theme.colors.surface, borderRadius: rp(24),
          padding: rp(20), shadowColor: theme.colors.primary, shadowOpacity: 0.06,
                ...
                <View key={`step-wrap-${i}`}>
                  ...
                  {step.rating_comment && (
                    <View style={{
                      ...
                      <Text style={{
                        fontFamily: theme.fontFamily.regular, color: theme.colors.success,
                        fontSize: rs(12),
                        fontStyle: "italic",
                      }}>
                        "{step.rating_comment}"
      ...
      <Modal visible={!!lightboxUrl} transparent
        ...
        >
          ...
          <Text style={{
            fontFamily: theme.fontFamily.regular, color: "rgba(255,255,255,0.5)",
